...
def pthw_corr_meth_bmindp(fld_pth, fl_nm, methlt_dt_fld, smp_info, **kwargs):
    '''
    - obtain the metabolic pathways which flux/protein usage correlated with DNA methylation independently of cell growth rate across the different cell lines
    ...
    # corrected by (divide by) biomass flux:
    rc_divided_by_bm = ndf2.drop('Subsystem', axis=0).divide(biomass, axis=0) # reaction flux/protein usage divided by biomass
    # aggregate by subsystem:
    sbsm_divided_by_bm = rc_divided_by_bm.append(pd.DataFrame(ndf2.loc['Subsystem', :]).T)
    sbsm_divided_by_bm = sbsm_divided_by_bm.T.groupby('Subsystem').mean().T # each average substm flux/protein usage divided by biomass
    # calculate coefficient and P-value:
    # with spearman correlation:
    res_spm = sbsm_divided_by_bm.apply(lambda x: spearmanr(x, pd.DataFrame(gb)), axis=0)  # NaN is when there is no variation
    res_spm.index = ['Coefficient', 'P-value']
    # with pearson correlation:
    res_prs = sbsm_divided_by_bm.apply(lambda x: pearsonr(x, pd.DataFrame(gb)), axis=0) # NaN is when there is no variation
    ...
    if 'flx' in fl_nm:
        ...
        el = 'prot'
    sig_sys_spm.to_csv(os.path.join(fld_pth, 'spearman', f'corr_meth_{el}_substm.tsv'), sep='\t')
    sig_sys_prs.to_csv(os.path.join(fld_pth, 'pearson', f'corr_meth_{el}_substm.tsv'), sep='\t')
    sig_sys_common_pos.to_csv(os.path.join(fld_pth, 'common_butcoefpearson', f'corr_meth_{el}_substm_pos.tsv'), sep='\t')
    sig_sys_common_neg.to_csv(os.path.join(fld_pth, 'common_butcoefpearson', f'corr_meth_{el}_substm_neg.tsv'), sep='\t')
...
def experimental_corr_test(methlt_dt_fld, trscp_pth, mth_trscp_corrvalue, envcond, gen_gecko_pth, ensembl_symbol_file,
    ...
    if div_growth:
        ...
        trscp = trscp.divide(biomass, axis=1)
    # get correlations:
    ...
    # select genes with significant correlation using both spearman and pearson and report their pearson p-values:
    common_sig = list(set(corr_trscp_mth_spm_sig.index).intersection(set(corr_trscp_mth_prs_sig.index)))
    ...
    if mth_trscp_corrvalue == 'direct':
        corr_same_sig_trscp = corr_same_sig[corr_same_sig['Coefficient'] > 0]
    elif mth_trscp_corrvalue == 'inverse':
        corr_same_sig_trscp = corr_same_sig[corr_same_sig['Coefficient'] < 0]
    # get the significant correlated genes:
    mth_trscp = set(corr_same_sig_trscp['gene_name'])
...
if '__name__' == '__main__':
    ### Pathways which flux/protein usage correlates with overall methylation independently of growth:
    FLD_PTH='results/ecGEM_simul_human1/init/no_tsks/no_flux_constr/biomass_constr/cl_spec_DNAtot/corr_pth_mth'
    ...
    # pathways which fluxes in average significantly correlates (direct/reverse) with DNA methylation level/growth rate:
    pthw_corr_meth_bmindp(fld_pth=FLD_PTH, fl_nm=FLX_FL_NM, methlt_dt_fld=METHLT_FLD, smp_info=ACHILLES_SMP_INFO)
    # pathways which protein usage in average significantly correlates (direct/reverse) with DNA methylation level/growth rate:
    pthw_corr_meth_bmindp(fld_pth=FLD_PTH, fl_nm=PRT_FL_NM, methlt_dt_fld=METHLT_FLD, smp_info=ACHILLES_SMP_INFO, fl_nm_bm=FLX_FL_NM)
